...
**v3.2.2** 功能优化与问题修复：
- 链接管理功能未开启时不再注册"查看历史记录"菜单项，避免用户混淆
- 修复"清空历史"按钮夜间模式显示问题，调整按钮样式为红色背景白色文字
- 默认不注册快捷键，改为空字符串，需用户手动设置后才生效
- 优化快捷键提示显示逻辑，未设置快捷键时不显示快捷键提示文字

**v3.2.1** 修复网页加载时dialog闪现黑框的css问题

**v3.2.0** 优化对话框隐藏逻辑，移除自动清空内容操作;调整hide方法，不再自动清空对话框内容和重置显示状态，改为由调用者自行处理相关清理工作，简化隐藏流程

**v3.1.9** 修复识别对话框显示空白问题

**v3.1.8** 代码结构改进：创建统一的SETTINGS_CONFIG配置对象，集中管理所有设置项，避免重复定义，提高代码可维护性

**v3.1.7** 修复智能检测网盘失效问题

**v3.1.5** 增强链接历史记录管理功能，添加重复链接检测逻辑，当检测到重复链接时自动删除原记录并添加新记录到开头，保持历史记录的整洁性和最新性
